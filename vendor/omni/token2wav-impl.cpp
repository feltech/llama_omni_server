...
bool flowGGUFModelRunner::init_from_host_caches(const flowStreamCacheHost & cache_host,
                                                ...
                                                float                       temperature) {
    if (!loader_.backend() || !loader_.model()) {
        LOG_ERROR("flowGGUFModelRunner.init_from_host_caches: loader backend or model is null\n");
        LOG_ERROR("  loader_.backend() = %p\n", (void*)loader_.backend());
        LOG_ERROR("  loader_.model() = %s\n", loader_.model() ? "valid" : "null");
        return false;
    }
    if (!spk_bc || B <= 0) {
        LOG_ERROR("flowGGUFModelRunner.init_from_host_caches: invalid spk_bc or B (spk_bc=%p, B=%lld)\n",
                  (void*)spk_bc, (long long)B);
        return false;
    }
    if (cache_host.empty()) {
