...
bool Token2WavSession::init_from_prompt_cache_gguf(const std::string & encoder_gguf,
    ...
    reset();
    std::fprintf(stderr, "[Token2Wav] Loading models...\n");
    if (!t2w.load_models(encoder_gguf, flow_matching_gguf, flow_extra_gguf, vocoder_gguf, device_token2mel,
                         device_vocoder)) {
        std::fprintf(stderr, "[Token2Wav] ERROR: load_models() failed\n");
        return false;
    }
    std::fprintf(stderr, "[Token2Wav] Models loaded successfully\n");
    std::fprintf(stderr, "[Token2Wav] Starting stream with prompt cache...\n");
    if (!t2w.start_stream_with_prompt_cache_gguf(prompt_cache_gguf_path, n_timesteps, temperature)) {
        std::fprintf(stderr, "[Token2Wav] ERROR: start_stream_with_prompt_cache_gguf() failed\n");
        return false;
    }
    std::fprintf(stderr, "[Token2Wav] Stream started successfully\n");
